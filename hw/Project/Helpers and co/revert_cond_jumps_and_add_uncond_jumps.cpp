...
public:
	...
	xed_ins_to_translate(ADDRINT new_addr, USIZE new_size, xed_error_enum_t& xed_code) : addr(new_addr), size(new_size) {
		xed_decoded_inst_zero_set_mode(&(data), &dstate);
		xed_code = xed_decode(&(data), reinterpret_cast<UINT8*>(addr), XED_MAX_INSTRUCTION_BYTES);
	...
	bool revert_cond_jump(xed_error_enum_t& xed_code) {
		...
		xed_error_enum_t xed_error = xed_encode(&xed_to_revert, enc_buf, max_size, &new_size);
...
xed_ins_to_translate* func() {
	// create a direct uncond jump to the same address:
	xed_uint8_t enc_buf[XED_MAX_INSTRUCTION_BYTES];		
	xed_int32_t disp = (xed_int32_t)(-3);
	...
	//xed_decoded_inst_zero_set_mode(&(result->data), &dstate);
	//this->data = new_xedd;
	//xed_error = ;
	if (xed_decode(&(result->data), enc_buf, XED_MAX_INSTRUCTION_BYTES) != XED_ERROR_NONE) {
		//xed_code = xed_error;
		return nullptr;
	}
	return result;
...
/*		if (xed_error != XED_ERROR_NONE) {
  ...
  //      
		...
		xed_ins_to_translate* result;
		char buf[2048];
		if (first) {
			result = func();
			if (result != nullptr) {
				xed_format_context(XED_SYNTAX_INTEL, &(result->data), buf, 2048, INS_Address(ins_tail), 0, 0);
				cerr << "orig at xed: " << hex << INS_Address(ins_tail) << " " << buf << endl;
			}
			else {
				cerr << "SHITTTT" << endl;
			}
			...
			xed_ins_to_translate xedd(INS_Address(ins_tail), INS_Size(ins_tail), xed_error);
			...
			if (!xedd.revert_cond_jump(xed_error))
				continue;
			if (xed_error != XED_ERROR_NONE) {
				cerr << "ERROR: xed decode failed for instr at: " << "0x" << hex << INS_Address(ins_tail) << endl;
				continue;
			}
			xed_format_context(XED_SYNTAX_INTEL, &(xedd.data), buf, 2048, INS_Address(ins_tail), 0, 0);
			cerr << "revert: " << hex << INS_Address(ins_tail) << " " << buf << endl << endl;
		}
    }
